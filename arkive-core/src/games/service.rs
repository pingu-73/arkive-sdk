...
impl GameService {
    ...
    pub fn new(
        ...
        let server_key = match (network, is_mutinynet) {
            (Network::Regtest, false) => {
                ...
                XOnlyPublicKey::from_str("4e79dab75861bf218443b5934afa2cd71d26c80104148e611f1093dbdfb4248d")
                    .expect("valid regtest server key")
            }
            (Network::Signet, true) => {
                // For Mutinynet - remove the '03' prefix
                XOnlyPublicKey::from_str("fa73c6e4876ffb2dfc961d763cca9abc73d4b88efcb8f5e7ff92dc55e9aa553d")
                    .expect("valid mutinynet server key")
            }
            (Network::Signet, false) => {
                // For regular Signet - remove the '02' prefix
                XOnlyPublicKey::from_str("8bf56160efc769112b361de4117b3c71b88ca16f1bb9f6ac7a2781929abc5e6a")
                    .expect("valid signet server key")
            }
            _ => {
                // Fallback for other networks
                tracing::warn!("Using coordinator key as server key for network {:?}", network);
                coordinator_key
            }
        };
    ...
    pub async fn load_lottery_escrow(&self, lottery_id: &str) -> Result<crate::LotteryEscrow> {
        self.lottery_coordinator.load_lottery_escrow(lottery_id).await
    }
    ...
    pub async fn create_lottery(&self, players: usize, entry_fee: Amount) -> Result<String> {
        use super::lottery::{LotteryConfig, TrustlessLottery};

        let (coordinator_key, _) = self.keypair.x_only_public_key();

        let config = LotteryConfig {
            min_players: players,
            max_players: players,
            entry_fee,
            commitment_timeout: 100,
            reveal_timeout: 100,
            coordinator_key,
            network: self.network,
        };

        let lottery = TrustlessLottery::new(config, self.storage.clone());

        // Generate participant keys for demo
        let participants = self.generate_participant_keys(players)?;

        let escrow = lottery.create_lottery(participants).await?;

        Ok(hex::encode(escrow.escrow_id))
    }
    ...
    pub async fn create_escrow_lottery(&self, players: usize, entry_fee: Amount) -> Result<String> {
        ...
        self.create_escrow_lottery_with_keys(participants, entry_fee).await
    }
    ...
    pub async fn create_escrow_lottery_with_keys(
        ...
        let lottery = self.lottery_coordinator
    ...
    fn generate_participant_keys(&self, count: usize) -> Result<Vec<XOnlyPublicKey>> {
        use bitcoin::key::Keypair;
        use bitcoin::secp256k1::{Secp256k1, SecretKey};
        ...
        for i in 0..count {
            ...
            let mut seed = [0u8; 32];
            seed[0] = i as u8;
            let secret = SecretKey::from_slice(&seed).map_err(|e| {
                crate::ArkiveError::internal(format!("Key generation failed: {}", e))
            })?;
            let keypair = Keypair::from_secret_key(&secp, &secret);
            let (xonly, _) = keypair.x_only_public_key();
            keys.push(xonly);
    ...
    pub fn generate_lottery_commitment(keypair: &bitcoin::key::Keypair, lottery_id: &str) -> Result<crate::Commitment> {
    ...
    async fn move_vtxo_to_escrow(
        ...
        let ark_service = participant_wallet.get_ark_service().await
        ...
        tracing::info!("Creating real Ark transaction to send {} sats to {}", 
                       amount.to_sat(), escrow_address_str);
        
        // Use the existing send method to create real Ark transaction
        let txid_str = ark_service.send(ark_address, amount).await
        ...
        self.wait_for_ark_processing(ark_service.clone(), &txid).await?;
    ...
    async fn wait_for_ark_processing(
        ...
        let mut attempts = 0;
        let max_attempts = 60; // Increase to 60 attempts with 2 second delays = 2 minutes timeout
        let poll_interval = std::time::Duration::from_secs(2);
        
        while attempts < max_attempts {
            // Check transaction status through Ark service
            match self.check_ark_transaction_status(ark_service.clone(), txid).await {
                Ok(true) => {
                ...
                Ok(false) => {
                    ...
                    tracing::debug!("Transaction {} still pending, attempt {}/{}", txid, attempts + 1, max_attempts);
    ...
    pub async fn fund_lottery_escrow_with_wallet(
        ...
        let lottery_escrow = self.lottery_coordinator.load_lottery_escrow(lottery_id).await?;
        ...
        tracing::info!("Moving {} sats to escrow Ark address: {}", amount.to_sat(), escrow_address_str);
        
        let txid = self.move_vtxo_to_escrow(
        ...
        let mut updated_lottery = self.lottery_coordinator.load_lottery_escrow(lottery_id).await?;
        ...
        self.lottery_coordinator.update_lottery_escrow(&updated_lottery).await?;
        
        tracing::info!("Successfully moved {} sats to escrow for lottery {} (tx: {})", 
                       amount.to_sat(), lottery_id, txid);
