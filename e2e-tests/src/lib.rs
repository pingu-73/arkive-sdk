...
impl TestEnvironment {
    ...
    async fn fund_ark_server(&self) -> Result<()> {
        let forfeit_address = self.get_forfeit_address().await?;
        info!(" Funding Ark server forfeit address: {}", forfeit_address);

        self.nigiri_faucet(&forfeit_address, 10.0).await?;
