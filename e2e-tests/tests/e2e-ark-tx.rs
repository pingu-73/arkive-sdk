...
async fn test_basic_boarding_and_ark_transaction() -> Result<()> {
    ...
    info!("Alice successfully boarded funds in round: {}", round_result);

    alice.wait_for_balance(100_000_000, 0, 60).await?;

    let (alice_confirmed, alice_pending) = alice.get_balance().await?;
    info!("Alice balance after boarding: {} confirmed, {} pending", 
          alice_confirmed.to_sat(), alice_pending.to_sat());
    ...
    alice.wait_for_balance(0, expected_alice_remaining, 30).await?;

    let (alice_after_send_confirmed, alice_after_send_pending) = alice.get_balance().await?;
    info!("Alice balance after sending: {} confirmed, {} pending", 
          alice_after_send_confirmed.to_sat(), alice_after_send_pending.to_sat());
    ...
    info!("Bob balance after receiving: {} confirmed, {} pending", 
          bob_confirmed.to_sat(), bob_pending.to_sat());

    assert_eq!(bob_pending.to_sat(), send_amount, "Bob should receive exactly what Alice sent");
    ...
    assert!(bob_round_result.is_some(), "Bob should have participated in a round");
    ...
    info!("Bob final balance: {} confirmed, {} pending", 
          bob_final_confirmed.to_sat(), bob_final_pending.to_sat());
    ...
    if alice_round_result.is_some() {
        ...
        alice.wait_for_balance(expected_alice_remaining, 0, 60).await?;
    } else {
    ...
    assert_eq!(total_remaining, 100_000_000, "Total funds should be conserved (minus fees)");

    info!("  Basic boarding and Ark transaction test completed successfully!");
    info!("  Final state:");
    info!("  Alice: {} sats (confirmed: {}, pending: {})", 
          alice_total, alice_final_confirmed.to_sat(), alice_final_pending.to_sat());
